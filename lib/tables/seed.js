...
const process = function (resource, callback) {
  var fullPath = `${path}${resource.id.replace('.gz', '.txt')}`;
  ...
  txtFile.on('finish', function () {
    console.log(`Downloaded and extracted ${resource.id} to ${path}`);

    pool.connect(function (err, client, done) {
      if (err) { throw err; }
      console.log(`Copying records from ${resource.id} to table: ${resource.table}`);
