...
var defaults = {
  ...
  creates: [],
  indexes: [
    'CREATE INDEX IF NOT EXISTS requests_by_timestamp ON requests (timestamp_year DESC, timestamp_month DESC, timestamp_day DESC, timestamp DESC);',
    'CREATE INDEX IF NOT EXISTS submissions_by_id ON submission_dim (id);',
    'CREATE INDEX IF NOT EXISTS submissions_by_id ON submission_fact (submission_id);',
    'CREATE INDEX IF NOT EXISTS assignment_override_by_assignment_id ON assignment_override_user_rollup_fact (assignment_id);',
    'CREATE INDEX IF NOT EXISTS enrollments_by_id ON enrollment_dim (id);'
  ],
  comments: []
};
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var item in schema) {
      ...
      for (var i in columns) {
        ...
        // if (table === 'requests' && coltype === 'uuid') { typeStr = 'uuid PRIMARY KEY'; }
